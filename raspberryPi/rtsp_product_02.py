    ...
    def update_status_from_message(self, data_num: int, data_value: int):
        ...
        with self.lock:
            if data_num == 0x0001:  # 現在階数
                ...
                if current_floor and current_floor != floor_name:
                    # 現在階と異なる階数設定の場合、行先階として設定
                    self.current_status['target_floor'] = floor_name
                    logger.info(f"🎯 階数設定により行先階を設定: {current_floor} → {floor_name} (データ値: {data_value:04X})")
                else:
                    # 同じ階または初回設定の場合、現在階として設定
                    self.current_status['current_floor'] = floor_name
                    # 到着時は行先階をクリア
                ...
                if current_floor and current_floor != floor_name:
                    # 現在階と異なる階数設定の場合、行先階として設定
                    self.current_status['target_floor'] = floor_name
                    logger.info(f"🎯 自動運転モード階数設定により行先階を設定: {current_floor} → {floor_name} (データ値: {data_value:04X})")
                else:
                    # 同じ階または初回設定の場合、現在階として設定
                    self.current_status['current_floor'] = floor_name
                    # 到着時は行先階をクリア
