...
class ElevatorENQSimulator:
    ...
    def initialize(self):
        """初期化"""
        logger.info("🏢 エレベーターENQ専用シミュレーター初期化（改良版）")
    ...
    def _send_enq(self, data_num: str, data_value: str, description: str):
        ...
        try:
            ...
            logger.info(f"[{timestamp}] 📤 ENQ送信: {description} (局番号:{station} データ:{data_value} チェックサム:{checksum}) [送信数:{self.total_sent}]")
            
        except Exception as e:
            self.total_errors += 1
            logger.error(f"❌ ENQ送信エラー: {e} [エラー数:{self.total_errors}]")
    ...
    def start_simulation(self):
        ...
        logger.info("🚀 エレベーターENQ専用シミュレーション開始（改良版）")
        logger.info("📋 送信仕様:")
        logger.info("   ①現在階 → ②行先階 → ③乗客降客 → 10秒待機 → ④着床")
        logger.info("   ①～③は2秒間隔で送信（受信安定性向上）")
        ...
        try:
            while self.running:
                ...
                self._send_enq("0001", current_hex, f"現在階: {self._floor_to_string(self.current_floor)}")
                logger.info("⏰ 2秒待機中...")
                time.sleep(2)  # 送信間隔を2秒に統一
                ...
                self._send_enq("0002", target_hex, f"行先階: {self._floor_to_string(target_floor)}")
                logger.info("⏰ 2秒待機中...")
                time.sleep(2)  # 送信間隔を2秒に統一
                ...
                # ③乗客降客送信
                self._send_enq("0003", "074E", "乗客降客: 1870kg")
                logger.info("⏰ 2秒待機中...")
                time.sleep(2)  # 送信間隔を2秒に統一
                ...
                # ④着床送信（行先階を0000に設定）
                self._send_enq("0002", "0000", "着床: 行先階クリア")
                ...
                # 統計情報表示
                logger.info(f"📊 統計: 送信数={self.total_sent}, エラー数={self.total_errors}")
    ...
    def stop_simulation(self):
        ...
        # 最終統計表示
        logger.info(f"📊 最終統計: 送信数={self.total_sent}, エラー数={self.total_errors}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='エレベーターENQ専用シミュレーター（改良版）')
